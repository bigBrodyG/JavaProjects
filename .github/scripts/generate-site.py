#!/usr/bin/env python3
"""Generate index.html for JavaProjects — Charcoal design, row-inline panels, tag filters."""
import json
import re
...
def classify_tags(source: str, name: str, category: str) -> list:
    tags = []
    c = category.lower()
    n = name.lower()

    # type from directory structure
    if 'verific' in c or 'verific' in n or 'esame' in n:
        tags.append('exam')
    elif 'laborator' in c:
        tags.append('lab')
    elif 'esercizi' in c:
        tags.append('esercizi')

    # OOP features
    if re.search(r'\bclass\b\s+\w+', source):
        tags.append('OOP')
    if ' extends ' in source:
        tags.append('extends')
    if ' implements ' in source:
        tags.append('implements')
    if re.search(r'\binterface\b\s+\w+', source):
    ...
    # data structures
    if 'ArrayList' in source:
        tags.append('ArrayList')
    if re.search(r'HashMap|TreeMap|LinkedHashMap', source):
        tags.append('HashMap')
    if re.search(r'\[\]\s', source):
        tags.append('arrays')
    if re.search(r'<[A-Z]\w*(?:,\s*[A-Z]\w*)*>', source):
        tags.append('generics')
    if re.search(r'\benum\b\s+\w+', source):
        tags.append('enum')

    # patterns
    if re.search(r'\btry\s*\{', source) and 'catch' in source:
        tags.append('exceptions')
    if 'Scanner' in source:
        tags.append('Scanner')
    if re.search(r'FileReader|BufferedReader|FileWriter|PrintWriter', source):
        tags.append('file IO')
    if re.search(r'Arrays\.sort|Collections\.sort|\.compareTo\b|Comparable', source):
        tags.append('sorting')

    # recursion heuristic: method calls itself
    methods = re.findall(r'(?:public|private|protected)\s+\w[\w<>\[\]]*\s+(\w+)\s*\(', source)
    for m in methods:
        if len(re.findall(rf'\b{re.escape(m)}\s*\(', source)) >= 2:
...
def find_projects(base: Path) -> list:
    ...
    def scan_dir(dir_path: Path, category: str):
        ...
        try:
            primary_src = primary.read_text(encoding='utf-8', errors='replace')
        except Exception:
            primary_src = ''
        # Collect all java source for tag detection
        all_src = '\n'.join(
            f.read_text(encoding='utf-8', errors='replace')
            for f in java_files[:8]
        )
        tags = classify_tags(all_src, name, category)
        ...
        if output_file.exists():
            ...
            if raw and 'No main method' not in raw and not raw.lower().startswith('error:'):
                output = raw
                has_output = True
        projects.append({
            'name': name,
...
def build_projects_json(projects: list) -> str:
    data = {}
    for p in projects:
        hl = wrap_lines(highlight_java(p['source_content']))
        entry = {
